...
const QuestionCard = ({ question, user, onAnswerPosted }) => {
  ...
  const handleSubmitAnswer = async (e) => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.message || error.message || 'Failed to post answer. Please try again.';
  ...
  const handleCustomTranslate = async () => {
    if (customLanguage.trim()) {
      setTranslating(true);
      try {
        const response = await translateAPI.translate(question.title, customLanguage);
        setTranslatedTitle(response.data.translated);
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex-1">
                  ...
                  <select
                    onChange={(e) => {
                      if (e.target.value === 'custom') {
                        ...
                        handleTranslateQuestion(e.target.value);
                      }
                    }}
            ...
            {showCustom && (
              <div className="mt-3 p-2 bg-white rounded border border-gray-300 space-y-2">
                <input
                  type="text"
        ...
        {translatedTitle && (
          <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg text-sm text-blue-800">
            <strong>Translated ({selectedLanguage?.toUpperCase()}):</strong> {translatedTitle}
          </div>
        )}
